...
No password ever touches this codebase, and there's no bundled/downloaded browser — it
uses whatever's already on your machine. The first time a tool needs a token and none is
cached, it starts a throwaway local HTTP server serving a login page and opens it in your
system's default browser. That page submits the login straight from your browser to
Memobot's own endpoint (its CORS is wide open, so this works) — the password never touches
this codebase — and then posts just the resulting token back to the local server. That
token is cached to `~/.config/memobot-mcp/credentials.json` (0600 permissions) and reused
for its ~1 year validity. When it expires (or an API call gets a 401), the server first
tries a silent refresh via the cached refresh_token (`POST /authen/api/v1/auth/token`) —
the interactive step only reappears if there's no cache at all or the refresh_token itself
has been revoked (e.g. after a password change).

On a machine with no display (a server reached over plain SSH, no X/Wayland), it can't
auto-open a browser, so it just prints the URL instead — open it in *any* browser,
anywhere (your own laptop/phone, using SSH port-forwarding if the server isn't local). Set
`MEMOBOT_MCP_HEADLESS=1` to force this print-only behavior even on a machine that does
have a display.
